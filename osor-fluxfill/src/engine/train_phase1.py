...
class Phase1Trainer:
    def __init__(self, config):
        ...
        logger.info(f"="*80)
        logger.info(f"Initializing Flux Removal Trainer - PHASE I (Flow Matching)")
        logger.info(f"="*80)
        ...
        def save_model_hook(models, weights, output_dir):
            if self.accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(self.unwrap_model(model), FluxGenerator):
                        weights.pop(i)
    ...
    def forward_generator(self):
        ...
        pred_latents = pred_latents * self.batch_inputs.mask_latent + latents_shot * (1 - self.batch_inputs.mask_latent)

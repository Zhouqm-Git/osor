"""
Phase I Trainer: one-step removal with hard latent blending (SDXL-Inpainting backbone).
"""
...
from src.utils.losses import MaskAwareGANLoss, MaskAwareL1Loss, LPIPSLoss
...
class Phase1Trainer:
    def __init__(self, config):
        ...
        logger.info(f"="*80)
        logger.info(f"Initializing OSOR SDXL Trainer - PHASE I (hard-blend one-step removal)")
        logger.info(f"="*80)
    ...
    def init_generator(self):
        """Initialize SDXL-Inpainting UNet generator with LoRA configuration (without cross-attention)"""
        logger.info(f"Loading base model from: {self.config.model.sdxl_base}")
        ...
        def save_model_hook(models, weights, output_dir):
            if self.accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(self.unwrap_model(model), DiffGANGenerator):
                        weights.pop(i)
    ...
    def prepare_batch_inputs(self, batch):
        ...
        self.batch_inputs = BatchInput(
            shot=shot, bg=bg, z_shot=z_shot, z=z, z_in=z_in, mask=mask, 
            timesteps_G=timesteps_G, mask_latent=mask_latent,
    ...
    def forward_generator(self):
        ...
        z = z * self.batch_inputs.mask_latent + self.batch_inputs.z_shot * (1 - self.batch_inputs.mask_latent)
    ...
    def optimize_generator(self):
        with self.accelerator.accumulate(self.G):
            ...
            loss_l1 = self.l1_loss(x, self.batch_inputs.bg, self.batch_inputs.mask) * self.config.loss_weights.l1
            loss_lpips = self.net_lpips(x, self.batch_inputs.bg) * self.config.loss_weights.lpips
            
            fake_logits, _ = self.D(x)
            loss_disc = self.gan_loss.generator_loss(fake_logits, self.batch_inputs.mask) * self.config.loss_weights.gan
            
            if not torch.isfinite(loss_l1).all() or not torch.isfinite(loss_disc).all():
                logger.error("Non-finite loss at step %d", self.global_step)
                raise RuntimeError("Non-finite encountered")

            loss_G = loss_l1 + loss_lpips + loss_disc
        ...
        return dict(G_total=loss_G, G_l1=loss_l1, G_lpips=loss_lpips, G_disc=loss_disc)
    ...
    def optimize_discriminator(self):
        ...
        with self.accelerator.accumulate(self.D):
            ...
            fake_logits, _ = self.D(x)
            loss_gan = self.gan_loss.discriminator_loss(real_logits, fake_logits, self.batch_inputs.mask)
    ...
    def run(self):
        ...
        logger.info("Starting Phase I Training (hard-blend one-step removal)")
    ...
    def log_grads(self):
        ...
        losses = [("l1", loss_l1), ("lpips", loss_lpips), ("disc", loss_disc)]
        grad_dict = {}
        self.G_opt.zero_grad()

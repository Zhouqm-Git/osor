"""
Phase II Trainer: alpha-aware removal with adaptive blending (SDXL-Inpainting backbone).
"""
...
class Phase2Trainer:
    def __init__(self, config):
        ...
        logger.info(f"="*80)
        logger.info(f"Initializing OSOR SDXL Trainer - PHASE II (alpha-aware removal)")
        logger.info(f"="*80)
    ...
    def init_scheduler(self):
        ...
        self.scheduler = DDPMScheduler.from_pretrained(self.config.model.sdxl_base, subfolder="scheduler")

    def init_vae(self):
        self.vae = AutoencoderKL.from_pretrained(
            self.config.model.sdxl_base, subfolder="vae", torch_dtype=self.weight_dtype).to(self.device)
    ...
    def init_generator(self):
        """Initialize Generator structure. Weights are loaded in on_training_start."""
        self.G = DiffGANGeneratorEnhance(
            base_model_path=self.config.model.sdxl_base,
        ...
        # Force LoRA params to fp32 for stability
        for p in filter(lambda p: p.requires_grad, self.G.parameters()):
            p.data = p.to(torch.float32)

    def init_discriminator(self):
        """Initialize Discriminator structure."""
        ctx = (
            nullcontext()
        ...
        def save_model_hook(models, weights, output_dir):
            if self.accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(self.unwrap_model(model), DiffGANGeneratorEnhance):
                        weights.pop(i)
    ...
    def on_training_start(self):
        """Initialize global step and load weights (Resume or Init Checkpoint)."""
        self.global_step = 0
        ...
        if self.config.checkpointing.resume_from_checkpoint:
            ...
            logger.info(f"Resumed full training state from {path}")

        elif self.config.checkpointing.init_checkpoint:
            ckpt_path = self.config.checkpointing.init_checkpoint
            logger.info(f"Initializing weights from Phase I: {ckpt_path}")
            
            g_path = os.path.join(ckpt_path, "state_dict.pth")
            state_dict_g = torch.load(g_path, map_location="cpu")

            if "unet.conv_out.weight" in state_dict_g:
                logger.info("Loading finetuned conv_out from Phase I (4ch -> 5ch expansion)")
                with torch.no_grad():
                     self.unwrap_model(self.G).unet.conv_out.weight.data[:4] = state_dict_g.pop("unet.conv_out.weight")
                     if "unet.conv_out.bias" in state_dict_g:
                         self.unwrap_model(self.G).unet.conv_out.bias.data[:4] = state_dict_g.pop("unet.conv_out.bias")
            
            # Load remaining weights (LoRA + others) with strict=False
            self.unwrap_model(self.G).load_state_dict(state_dict_g, strict=False)
            logger.info(f"Loaded G from {g_path}")

            d_path = os.path.join(ckpt_path, "model.safetensors") 
            if os.path.exists(d_path):
                self.unwrap_model(self.D).load_state_dict(load_file(d_path), strict=True)
                logger.info(f"Loaded D from {d_path}")
            else:
                logger.warning(f"No Discriminator weights found at {d_path}. D initialized from scratch.")
    ...
    def prepare_batch_inputs(self, batch):
        ...
        z_shot = self.vae.encode(shot.to(self.weight_dtype)).latent_dist.sample()
        z_shot = z_shot * self.vae.config.scaling_factor    
        bs = z_shot.shape[0]

        mask_latent = F.max_pool2d(mask, kernel_size=8, stride=8).to(device=self.device, dtype=self.weight_dtype)
        gt_mask_latent = F.max_pool2d(gt_mask, kernel_size=8, stride=8).to(device=self.device, dtype=self.weight_dtype)

        noise = torch.randn_like(z_shot, device=self.device, dtype=self.weight_dtype)
        timesteps_G = torch.full((bs,), self.config.model.model_t, dtype=torch.long, device=self.device)
        z = self.scheduler.add_noise(z_shot, noise, timesteps_G)
        z_in = torch.cat([z, mask_latent, z_shot], dim=1)
        ...
        ], dtype=z_shot.dtype, device=z_shot.device).repeat(z_shot.shape[0], 1)
        ...
        self.batch_inputs = BatchInput(
            shot=shot, bg=bg, z_shot=z_shot, z=z, z_in=z_in, mask=mask, gt_mask=gt_mask, noise=noise, 
            timesteps_G=timesteps_G, mask_latent=mask_latent, gt_mask_latent=gt_mask_latent,
    ...
    def forward_generator(self):
        eps, mask_logits = self.G(
            self.batch_inputs.z_in,
            self.batch_inputs.timesteps_G,
            added_cond_kwargs=self.batch_inputs.added_cond_kwargs,
            encoder_hidden_states=self.batch_inputs.prompt_embeds
        )
        alpha = torch.sigmoid(mask_logits)
        
        z = self.scheduler.step(eps, self.config.model.model_t, self.batch_inputs.z).pred_original_sample
        
        z = z * alpha + self.batch_inputs.z_shot * (1 - alpha)
